...
public class NetworkValidator {
	...
	public boolean isContainedInNetwork() {
		...
		for (int i = 0; i < subnetParts.length; ++i) {
			if ((Integer.parseInt(ipOne[i]) & Integer.parseInt(subnetParts[i])) != (Integer.parseInt(ipTwo[i])
					& Integer.parseInt(subnetParts[i])))
				return false;
		}
		return true;
	}
	...
	public void validate() {
		while (true) {
			...
			setSubnetMask();
			setSecondIP();
			System.out.println("Your second IP Address"
					+ (isContainedInNetwork() ? " is a part of the network." : " is not a part of the network.\n"));
			resetValues();
		}
	}
